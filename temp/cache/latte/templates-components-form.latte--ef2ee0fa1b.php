...
/** source: D:\xampp\htdocs\articlesite\app\Presenters\templates\components\form.latte */
final class Templateef2ee0fa1b extends Latte\Runtime\Template
{
...
';
	...
	public function blockForm(array $ʟ_args): void
	{
		...
		$form = $this->global->formsStack[] = is_object($formName) ? $formName : $this->global->uiControl[$formName];
		echo '	<form';
...
';
		...
		foreach ($form->controls as $input) {
			if (!$input->getOption('rendered') && $input->getOption('type') !== 'hidden') {
				echo '	<tr';
				echo ($ʟ_tmp = array_filter([$input->required ? 'required' : null])) ? ' class="' . LR\Filters::escapeHtmlAttr(implode(" ", array_unique($ʟ_tmp))) . '"' : "";
				echo '>
...
';
		...
		foreach ($form->controls as $name => $input) {
			if (!$input->getOption('rendered') && $input->getOption('type') !== 'hidden') {
				echo '	<div';
				echo ($ʟ_tmp = array_filter(['form-group', $input->required ? 'required' : null, $input->error ? 'has-error' : null])) ? ' class="' . LR\Filters::escapeHtmlAttr(implode(" ", array_unique($ʟ_tmp))) . '"' : "";
				echo '>
...
';
				...
				elseif ($input->getOption('type') === 'checkbox') {
					...
					$_input = is_object($input) ? $input : end($this->global->formsStack)[$input];
					echo $_input->getControl() /* line 39 */;
					echo '</div>
';
				}
				elseif ($input->getOption('type') === 'radio') {
					...
					$_input = is_object($input) ? $input : end($this->global->formsStack)[$input];
					echo $_input->getControl() /* line 41 */;
					echo '</div>
';
				}
				else {
